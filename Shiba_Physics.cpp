...
Shiba_Physics::Shiba_Physics( Game_Renderer* renderer, Component_Game_Actor& shiba ) :
    Abstract_Actor_Physics( renderer->GetScreenHeight(), renderer->GetScreenWidth(), shiba.GetGraphics()->GetDestination().x, shiba.GetGraphics()->GetDestination().y, SHIBA_WIDTH, SHIBA_HEIGHT,
                            MAX_SPEED, MAX_CLIMB, MAX_FALL, MAX_ACCEL, MAX_DECEL, MAX_JUMP_ACCEL, MAX_JUMP_ADD, MAX_JUMP_HEIGHT )
{
}


Shiba_Physics::~Shiba_Physics()
{
}

void Shiba_Physics::Move( Component_Game_Actor& actor, Map &map, float timeStep )
{        
