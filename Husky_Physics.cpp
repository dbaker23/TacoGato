...
Husky_Physics::Husky_Physics( Game_Renderer* renderer, Component_Game_Actor& husky ):
    Abstract_Actor_Physics( renderer->GetScreenHeight(), renderer->GetScreenWidth(), husky.GetGraphics()->GetDestination().x, husky.GetGraphics()->GetDestination().y, HUSKY_WIDTH, HUSKY_HEIGHT,
                            MAX_SPEED, MAX_CLIMB, MAX_FALL, MAX_ACCEL, MAX_DECEL, MAX_JUMP_ACCEL, MAX_JUMP_ADD, MAX_JUMP_HEIGHT )
{
}


Husky_Physics::~Husky_Physics()
{
}

void Husky_Physics::Move( Component_Game_Actor& actor, Map &map, float timeStep )
{        
